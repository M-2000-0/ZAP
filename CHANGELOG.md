# Zap Language Changelog

All notable changes to the Zap programming language.

## [0.2.0] - 2026-07-28

### Added
- **Self-hosting interpreter**: ~95% of interpreter written in Zap (lexer, parser, evaluator, AST, environment, builtins)
...
- **Grammar versioning**: `# zap:grammar=0.2` pragma with mismatch detection
- **LSP server**: `zap lsp` for IDE integration (hover, goto-def, diagnostics)
- **Package manager**: `zap init`, `zap add`, `zap install` with `zap.json` config
- **WASM transpiler**: `wasm/zap-to-js.js` compiles Zap to JavaScript
...
### Added
- Initial Zap language implementation
...
- Example programs: hello, fibo, blog, rest_api, ai_native, game, zapphysics
