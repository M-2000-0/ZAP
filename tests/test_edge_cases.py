"""Edge case and negative tests for Zap lexer, parser, and evaluator."""
...
ZAP_CMD = [sys.executable, "-m", "src", "run"]


def run_zap(source):
    """Run Zap source code and return (returncode, stdout, stderr)."""
    with tempfile.TemporaryDirectory() as tmpdir:
        path = os.path.join(tmpdir, "test.zap")
...
class TestLexerEdgeCases:
    ...
    def test_only_comments(self):
        rc, stdout, stderr = run_zap("# just a comment\n# another\n")
        assert rc == 0

    def test_scientific_notation(self):
        rc, stdout, stderr = run_zap('print(1.5e3)\n')
        assert rc == 0
        assert "1500" in stdout

    def test_negative_scientific(self):
        rc, stdout, stderr = run_zap('print(1e-3)\n')
        assert rc == 0
        assert "0.001" in stdout

    def test_float_literal(self):
        rc, stdout, stderr = run_zap('print(3.14)\n')
        assert rc == 0
        assert "3.14" in stdout

    def test_string_escape_newline(self):
        rc, stdout, stderr = run_zap('print("line1\\nline2")\n')
        assert rc == 0
        assert "line1" in stdout and "line2" in stdout

    def test_string_escape_tab(self):
        rc, stdout, stderr = run_zap('print("col1\\tcol2")\n')
        assert rc == 0
        assert "\t" in stdout

    def test_string_escape_quote(self):
        rc, stdout, stderr = run_zap('print("he\\"llo")\n')
        assert rc == 0, stderr
...
class TestStringInterpolation:
    ...
    def test_simple_interpolation(self):
        rc, stdout, stderr = run_zap('let name = "World"\nprint("Hello ${name}")\n')
        assert rc == 0
        assert "Hello World" in stdout

    def test_interp_expression(self):
        rc, stdout, stderr = run_zap('print("2 + 2 = ${2 + 2}")\n')
        assert rc == 0
        assert "2 + 2 = 4" in stdout

    def test_interp_dollar_var(self):
        rc, stdout, stderr = run_zap('let x = 42\nprint("value is $x")\n')
        assert rc == 0
...
class TestControlFlowEdgeCases:
    ...
    def test_empty_list(self):
        rc, stdout, stderr = run_zap("print([])\n")
        assert rc == 0
        assert "[]" in stdout
...
class TestOperatorEdgeCases:
    ...
    def test_string_repeat(self):
        rc, stdout, stderr = run_zap('print("ha" * 3)\n')
        assert rc == 0
        assert "hahaha" in stdout

    def test_list_concat(self):
        rc, stdout, stderr = run_zap("print([1, 2] + [3, 4])\n")
        assert rc == 0
...
class TestLambdaAndComprehensions:
    ...
    def test_simple_lambda(self):
        rc, stdout, stderr = run_zap("let double = x => x * 2\nprint(double(5))\n")
        assert rc == 0, stderr
        assert "10" in stdout
...
class TestBuiltinEdgeCases:
    ...
    def test_type_builtin(self):
        rc, stdout, stderr = run_zap("print(type(42))\n")
        assert rc == 0, stderr
        assert "int" in stdout.lower() or "Int" in stdout

    def test_isinstance_builtin(self):
        rc, stdout, stderr = run_zap("print(isinstance(42, 'int'))\n")
        assert rc == 0, stderr
        assert "False" in stdout or "true" in stdout.lower()

    def test_float_conversion(self):
        rc, stdout, stderr = run_zap("print(float('3.14'))\n")
        assert rc == 0
        assert "3.14" in stdout

    def test_str_upper_lower(self):
        rc, stdout, stderr = run_zap('print(upper("hello"))\n')
        assert rc == 0
        assert "HELLO" in stdout

    def test_str_strip(self):
        rc, stdout, stderr = run_zap('print(strip("  hi  "))\n')
        assert rc == 0
        assert "hi" in stdout

    def test_abs_negative(self):
        rc, stdout, stderr = run_zap("print(abs(-10))\n")
        assert rc == 0
        assert "10" in stdout

    def test_round_float(self):
        rc, stdout, stderr = run_zap("print(round(3.7))\n")
        assert rc == 0
        assert "4" in stdout

    def test_len_empty_list(self):
        rc, stdout, stderr = run_zap("print(len([]))\n")
        assert rc == 0
        assert "0" in stdout
